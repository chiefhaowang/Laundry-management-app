...
<div>
  <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>物品清洗下单</el-breadcrumb-item>
        </el-breadcrumb>
  <el-container>
    <el-aside width="70%">
      <el-table
      ...
      <div class="grid-content bg-purple">
        <el-card class="box-card">
            ...
            <div class="personal-relation">
                <div class="relation-item">{{orderMenu}}<div style="float: right; padding-right:20px;"></div></div>
            </div>
  ...
  <!-- 新增订单确认弹窗 -->
        <el-dialog title="洗衣下单" :visible.sync="addDialogVisible" width="50%">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="addOrderDia">线上支付</el-button>
            </span>
        </el-dialog>
</div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        temPrice:0,
        orderMenu:"",
        tableData:[{size:"小号",price:0,number:0,isAble:false},{size:"中号",price:0,number:0,isAble:false},{size:"大号",price:0,number:0,isAble:false}],
    ...
    methods:{
    async getPriceList(){
			const {data:res_stu}=await this.$http.get("/clientstuffprice");
			this.stuffPrice=res_stu;
			//console.log(this.stuffPrice[1].name);
		},
    ...
    showOrderDialog(item){
      ...
      this.tableData[0].price=item.small;
      this.tableData[1].price=item.mid;
      this.tableData[2].price=item.big;
      for(var i=0;i<3;i++){
    ...
    async addOrderDia(){
      ...
      //订单内容填充
      this.addForm.content=this.orderMenu.substr(0, this.orderMenu.length - 1);
      ...
      this.payForm.id=this.addForm.id;
      this.payForm.price=this.addForm.orderpay;

      const {data:res2} = await this.$http.post("/alipay",this.payForm);
    ...
    getInfo(){
      ...
      this.addForm.address=us.address;
    },
