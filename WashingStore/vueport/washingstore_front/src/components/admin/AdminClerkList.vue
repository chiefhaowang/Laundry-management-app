    ...
    <div>
        ...
        <el-card>
            <el-row :gutter="25">
                <el-col :span="10">
                    <!-- 搜索区域 -->
                    <el-input placeholder="搜索内容" v-model="queryInfo.query" clearable @clear="getClerkList">

                    <el-button slot="append" icon="el-icon-search" @click="getClerkList"></el-button>
                    </el-input>
                </el-col>
                <el-col :span="4">
            ...
            <!-- 客户列表展示 -->
            <el-table :data="clerkList" border stripe>
                <!-- 索引 -->
                ...
                <el-table-column label="操作">
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button @click="deleteClerk(scope.row.id)" type="danger" icon="el-icon-delete" size="mini"></el-button>
                        </el-tooltip>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <!-- 新增用户区域 -->
        <el-dialog title="添加店员" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
...
export default {
    ...
    methods:{
        ...
        async getClerkList(){
            ...
            const {data:res} = await this.$http.get("/clerkuser",{params:this.queryInfo});
            this.clerkList = res.data;//用户列表数据封装
        ...
        addClerk(){
            this.$refs.addFormRef.validate(async valid=>{
                ...
                if(this.addForm.emailnew==this.sendedCode){
                    const {data:res}=await this.$http.post("addclerk",this.addForm);
                    if(res!="success"){
        ...
        async deleteClerk(id){
            ...
            const {data:res} = this.$http.delete("deleteclerk?id="+id);
            if(res=="error"){
        ...
        async showEditDialog(id){
            const {data:res} = await this.$http.get("updateclerk?id="+id);
            this.editForm=res;
            console.log(res);
        ...
        editClerk(){
          this.$refs.editFormRef.validate(async valid =>{
              if(!valid) return;
              const {data:res} = await this.$http.put("editclerk",this.editForm);
